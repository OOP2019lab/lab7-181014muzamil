...
bankAccount ** readFile (string); 
void printAccounts(bankAccount ** arr,int size);
bool makeTransaction(bankAccount ** & arr,int ID,float amount,string transtype,int size);
...

{
	...
	*/
	...
	b2[0]=new bankAccount(30,2000,trans,3);
	...
	*/
	...
	b3[0]=new bankAccount(50,2000,trans2,2);
	...
	*/
	bankAccount **b=readFile("data.txt");
	/********************************TASK 7*************************************
...
bankAccount ** readFile(string filepath)
{
	...
	else
 {
	int noID;
	fin>>noID;
	bankAccount ** b=new bankAccount * [noID];
		...
		int notrans;
		fin>>notrans;
	float * trans=new float[notrans];
	for(int j=0;j<notrans;j++)
	  {
     	fin>>trans[j];
	  }
	b[i]=new bankAccount(id,balance,trans,notrans,limit);
	}	
		return b;
}
}
...
bool makeTransaction(bankAccount ** & arr,int ID,float amount,char transtype,int size)
{
	for(int i=0;i<size;i++)
	{
		if(arr[i]->getID()==ID)
		{
		  ...
		  {
			  bool flag1=arr[i]->deposit(amount);
		      if(flag1==1)
				  return true;
			  else
				  return false;
		  ...
		  {
			  bool flag2=arr[i]->withdraw(amount);
		      if(flag2==1)
				  return true;
			  else
				  return false;
...
for(int i=0;i<size;i++)
{
	if(arr[i]->getbalance==0||arr[i]->getnotrans==0)
	{
	delete [] arr[i];
	}
}
